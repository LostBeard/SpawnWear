...
- **Link to constraints.** Every plan should reference the production constraints it's bound by: the AMOLED power model, the CO5300 alignment quirks. The constraint files live in `Research/` and `Notes/`. (NOTE: the old "deploy ceiling" is no longer a constraint — RESOLVED 2026-06-25, full 2.94 MB partition usable; see `Research/nf-interpreter-deploy-ceiling.md`.)
